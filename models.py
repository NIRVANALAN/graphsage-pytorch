...
class GSSupervised(nn.Module):
    def __init__(self,
        ...
        self.train_sample_fns = [partial(
            self.train_sampler, n_samples=s) for s in layer_specs[0]['train_samples']]
        self.val_sample_fns = [
            partial(self.val_sampler, n_samples=s) for s in layer_specs[0]['val_samples']]
        ...
        self.lr_scheduler = partial(
            getattr(LRSchedule, lr_schedule), lr_init=lr_init)
        self.lr = self.lr_scheduler(0.0)
        self.optimizer = torch.optim.Adam(
            self.parameters(), lr=self.lr, weight_decay=weight_decay)

    def forward(self, ids, feats, train=True, random_sample_neibs=True):
        # Sample neighbors
        ...
        # return identity feats as tmp_feats if using identity prep
        ...
        if random_sample_neibs:
            expanded_all_feats = list(all_feats)
            for neib_feat_idx in range(2, all_feats.__len__()):
                rand_neibs_feat_idx = torch.randperm(
                    all_feats[neib_feat_idx-1].shape[0])
                expanded_all_feats[neib_feat_idx-1] = torch.cat((
                    all_feats[neib_feat_idx-1], all_feats[neib_feat_idx][rand_neibs_feat_idx]))
            pass
        ...
        if random_sample_neibs:
            ...
            all_feats = expanded_all_feats
            for agg_layer in self.agg_layers.children():
                all_feats = [agg_layer(all_feats[k], all_feats[k + 1])
        ...
        # ?? Do we actually want this? ... Sometimes ...
